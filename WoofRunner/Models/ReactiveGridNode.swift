...
class ReactiveGridNode {
    ...
    init(_ gridVM: GridViewModel) {
        ...
        self.position = SCNVector3(Float(gridVM.gridPos.value.getCol()) * GameSettings.TILE_WIDTH,
                                   Float(gridVM.gridPos.value.getRow()) * GameSettings.TILE_WIDTH, 0)
        ...
        gridVM.gridPos.asObservable()
            .subscribe(onNext: {
                (pos) in
                self.position = SCNVector3(Float(pos.getCol()) * GameSettings.TILE_WIDTH, Float(pos.getRow()) * GameSettings.TILE_WIDTH, 0)
                self.platformNode.value.position = self.position
                self.obstacleNode.value.position = self.position + SCNVector3(0, 0, GameSettings.TILE_WIDTH)
